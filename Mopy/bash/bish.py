...
def importRacialEyesHair(srcMod,srcRaceEid,dstMod,dstRaceEid):
    ...
    if dstMod.lower() == 'oblivion.esm':
        raise exceptions.BoltError(u"You don't REALLY want to overwrite Oblivion.esm, do you?")
    ...
    for record in dstFile.RACE.records:
        if record.eid == dstRaceEid:
            dstRace = record
            break
    if not srcRace: raise exceptions.ModError(srcMod,u"Didn't find race (eid) %s." % srcRaceEid)
    if not dstRace: raise exceptions.ModError(dstMod,u"Didn't find race (eid) %s." % dstRaceEid)
    #--Get mapper
