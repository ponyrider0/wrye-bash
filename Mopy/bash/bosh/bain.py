...
class Installer(object):
    ...
    def _fixme_drop__for_loading_in_previous_versions(self):
        ...
        if isinstance(self, InstallerArchive):
            self.fileSizeCrcs = [(unicode(x), y, z) for x, y, z in
                                 self.fileSizeCrcs]
        elif isinstance(self, InstallerProject):
            InstallerProject._fileSizeCrcs = lambda x: []
    ...
    def _find_root_index(self, _os_sep=os_sep, skips_start=_silentSkipsStart):
        ...
        fileSizeCrcs = sorted(self.fileSizeCrcs,key=sort_keys_dict.__getitem__)
    ...
    def _fs_install(self, dest_src, srcDirJoin, progress,
        ...
        for dest, src in dest_src.iteritems():
            ...
            if srcFull.exists(): ## drop this syscall !
                ...
                data_sizeCrcDate_update[dest] = (size, crc, -1)
                srcs.append(srcFull)
...
class InstallerProject(Installer):
    ...
    def fileSizeCrcs(self):
        return self._fileSizeCrcs()

    def _fileSizeCrcs(self):
...
class InstallersData(DataStore):
    ...
    def _editTweaks(tweaksCreated):
        ...
        for (tweakPath, iniAbsDataPath) in tweaksCreated:
            ...
            iniInfos.refreshFile(tweakPath.tail) ## fixme add to BAIN ?
        ...
        * Install missing files from active anPackages."""
        ...
        for package in anPackages:
            ...
            if installer.isActive:
                removes |= installer.missingFiles ##: why?
